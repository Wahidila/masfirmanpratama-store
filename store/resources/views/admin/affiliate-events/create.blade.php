...
@section('title', 'Tambah Event Affiliate')

@section('content')
    <x-admin.page-header
        title="Tambah Event"
        subtitle="Buat event affiliate baru.">
        <x-slot name="actions">
    ...
    <x-admin.card>
        <form method="POST" action="{{ route('admin.affiliate-events.store') }}" class="p-5 space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                    class="mt-1 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="reward_note" name="reward_note" rows="2"
                    class="mt-1 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">{{ old('reward_note') }}</textarea>
                @error('reward_note')
                    <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                <x-admin.button type="submit" size="sm">Simpan Event</x-admin.button>
            </div>
        </form>
    </x-admin.card>
@endsection
